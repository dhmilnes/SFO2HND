...
OK looks like straight estimates - last visit and last purchase have some predictive ability

```{r logistic on prior visit}
pY <- glm(ret_purch~visit_p+purch_p, data=WVtrain, family="binomial")
summary(pY)
pred <- predict(pY, newdata=WVtest, type="response")
auc.fn(pred,WVtest$ret_flag)
...
```{r logistic purch conditioned}
test <- glmer(ret_purch~visit_p+purch_p+(1|user_hash), data=WVtrain, family="binomial")
summary(test)
WVtestA <- WVtest[WVtest$user_hash %in% WVtrain$user_hash,]
pred1 <- predict(test,newdata=WVtestA,type="response")
auc.fn(pred1,WVtestA$ret_flag)
...
Let's generate purchase predictions given a visit

```{r purchase given visit}
prates <- ddply(WVtrain, .(user_hash), summarize, 
          purch_session = sum(purch_c)/sum(visit_c),
          prob_purch_given_visit = sum(ret_purch)/sum(ret_flag))


head(prates,20)

WVtrain[WVtrain$user_hash=="00035b86e6884589ec8d28fbf2fe7757",]
